		...
		<div id="list-group" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="startTime" title="${message(code: 'group.startTime.label', default: 'Start Time')}" />
					
						<g:sortableColumn property="endTime" title="${message(code: 'group.endTime.label', default: 'End Time')}" />
				...
				<g:each in="${groupInstanceList}" status="i" var="groupInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${groupInstance.id}">${fieldValue(bean: groupInstance, field: "startTime")}</g:link></td>
					
						<td>${fieldValue(bean: groupInstance, field: "endTime")}</td>
